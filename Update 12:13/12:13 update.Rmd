...
So far, I only have results for rvTDT. Generally, it seems like .
...
#### RV-TDT: windows of size 100, filtered to CADD>10, in peak and in whole region

I also tried using windows and filtering on RV-TDT over the summer. I first filtered to all positions with a CADD score >10, since that annotation score had the most non-missing entries in the region. I then used relatively large windows of 76 or 100 SNPs, with either no overlap or a 10 SNP overlap.
...
##### QUESTION/TODO: Does this imply that, of the rare variants transmitted, only a small number are associated with the phenotype? Or that an individual that transmitted an associated rare variant transmitted only a few rare variants?
...
##### QUESTION/TODO: How to interpret the information in light of the fact that VT maximizes the test statistic over the allele frequencies? Does this imply that there may be an ?
